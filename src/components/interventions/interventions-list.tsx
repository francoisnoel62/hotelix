...
import { InterventionWithRelations, TechnicienOption } from '@/lib/types/intervention'
...
export function InterventionsList({
  ...
  const handleBulkActions = {
    updateStatut: async (ids: number[], statut: StatutIntervention) => {
      // Mise à jour optimiste pour tous les IDs
      ids.forEach(id => {
        onOptimisticUpdate(id, { statut })
      })

      try {
        const result = await updateMultipleInterventionStatut(ids, statut, user.id)
        if (result.success) {
          toast({
            variant: 'success',
            title: 'Statut mis à jour',
        ...
        toast({
          variant: 'error',
          title: 'Erreur',
          description: 'Erreur lors de la mise à jour en lot'
        })
      }
    },

    assignTechnician: async (ids: number[], technicienId: number | null) => {
      // Mise à jour optimiste pour tous les IDs
      const technicien = technicienId ? techniciens.find(t => t.id === technicienId) : null
      ids.forEach(id => {
      ...
      try {
        const result = await assignMultipleInterventions(ids, technicienId, user.id)
        if (result.success) {
          toast({
            variant: 'success',
            title: 'Assignation mise à jour',
    ...
    delete: async (ids: number[]) => {
      try {
        const result = await deleteMultipleInterventions(ids, user.id)
        if (result.success) {
  ...
  return (
    <div className="bg-white rounded-lg shadow">
      ...
      {/* Contenu avec rendu conditionnel selon la vue */}
      {viewMode === 'detailed' ? (
        ...
        <div className="p-4">
          <InterventionsTableView
            ...
            onTechnicienChange={handleTechnicienChange}
